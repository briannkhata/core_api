...
func main() {
	...
	// Run migrations
	if err := database.RunMigrations(db, "performance"); err != nil {
	...
	go func() {
		...
		logger.Info("Shutting down performance...")
	...
	// Start server
	logger.Info("Performance service starting on port %s", cfg.Port)
...
func setupRoutes(app *fiber.App, db interface{}) {
	...
	app.Get("/", func(c *fiber.Ctx) error {
		return c.JSON(fiber.Map{
			"message":    "YathuERP Performance Service is running",
			"service":    "performance",
			"go_version": "1.22",
		})
	})
