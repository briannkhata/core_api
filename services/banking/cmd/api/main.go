...
func main() {
	...
	// Run migrations
	if err := database.RunMigrations(db, "banking"); err != nil {
	...
	go func() {
		...
		logger.Info("Shutting down banking...")
	...
	// Start server
	logger.Info("Banking service starting on port %s", cfg.Port)
...
func setupRoutes(app *fiber.App, db interface{}) {
	...
	app.Get("/", func(c *fiber.Ctx) error {
		return c.JSON(fiber.Map{
			"message":    "YathuERP Banking Service is running",
			"service":    "banking",
			"go_version": "1.22",
		})
	})
