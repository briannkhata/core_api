...
func main() {
	...
	// Run migrations
	if err := database.RunMigrations(db, "organization"); err != nil {
	...
	go func() {
		...
		logger.Info("Shutting down organization...")
	...
	// Start server
	logger.Info("Organization service starting on port %s", cfg.Port)
...
func setupRoutes(app *fiber.App, db interface{}) {
	...
	app.Get("/", func(c *fiber.Ctx) error {
		return c.JSON(fiber.Map{
			"message":    "YathuERP Organization Service is running",
			"service":    "organization",
			"go_version": "1.22",
		})
	})
