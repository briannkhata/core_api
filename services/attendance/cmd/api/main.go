...
func main() {
	...
	// Run migrations
	if err := database.RunMigrations(db, "attendance"); err != nil {
	...
	go func() {
		...
		logger.Info("Shutting down attendance...")
	...
	// Start server
	logger.Info("Attendance service starting on port %s", cfg.Port)
...
func setupRoutes(app *fiber.App, db interface{}) {
	...
	app.Get("/", func(c *fiber.Ctx) error {
		return c.JSON(fiber.Map{
			"message":    "YathuERP Attendance Service is running",
			"service":    "attendance",
			"go_version": "1.22",
		})
	})
