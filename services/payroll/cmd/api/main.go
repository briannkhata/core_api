...
func main() {
	...
	// Run migrations
	if err := database.RunMigrations(db, "payroll"); err != nil {
	...
	go func() {
		...
		logger.Info("Shutting down payroll...")
	...
	// Start server
	logger.Info("Payroll service starting on port %s", cfg.Port)
...
func setupRoutes(app *fiber.App, db interface{}) {
	...
	app.Get("/", func(c *fiber.Ctx) error {
		return c.JSON(fiber.Map{
			"message":    "YathuERP Payroll Service is running",
			"service":    "payroll",
			"go_version": "1.22",
		})
	})
