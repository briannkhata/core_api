...
func main() {
	...
	// Run migrations
	if err := database.RunMigrations(db, "leave-service"); err != nil {
	...
	go func() {
		...
		logger.Info("Shutting down leave-service...")
	...
	// Start server
	logger.Info("Leave service starting on port %s", cfg.Port)
...
func setupRoutes(app *fiber.App, db interface{}) {
	...
	app.Get("/", func(c *fiber.Ctx) error {
		return c.JSON(fiber.Map{
			"message":    "YathuERP Leave Service is running",
			"service":    "leave-service",
			"go_version": "1.22",
		})
	})
